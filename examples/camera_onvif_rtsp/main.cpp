...
bool containsNal(const recamera::EncodedFrame& frame, std::uint8_t wanted) {
    for (const auto& block : frame.blocks()) {
        for (std::size_t i = 0; i + 4 < block.size(); ++i) {
            std::size_t header = 0;
            if (block[i] == 0 && block[i + 1] == 0 && block[i + 2] == 1) header = i + 3;
            else if (i + 4 < block.size() && block[i] == 0 && block[i + 1] == 0 &&
                     block[i + 2] == 0 && block[i + 3] == 1) header = i + 4;
            if (header != 0 && (block[header] & 0x1fU) == wanted) return true;
        }
    }
    return false;
}
...
int main() {
    ...
    std::fprintf(stderr, "RTSP: rtsp://%s:%u%s (Digest user=%s)\n",
                 onvifConfig.address.c_str(), rtspConfig.port, rtspConfig.path.c_str(), username);
