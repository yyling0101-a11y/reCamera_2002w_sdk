...
class CVI_RTSP: public RTSPServer {
    public:
        static CVI_RTSP* createNew(UsageEnvironment *env, CVI_RTSP_CONFIG *config, UserAuthenticationDatabase *authDatabase);
        virtual ~CVI_RTSP();
        void setListener(ConnCallback *connCb, void *argConn, ConnCallback *disconnCb, void *argDisconn);
    protected:
        CVI_RTSP(UsageEnvironment& env, int ourSocket, Port ourPort, unsigned timeout, const CVI_RTSP_CONFIG &config, UserAuthenticationDatabase *authDatabase);
...
CVI_RTSP* CVI_RTSP::createNew(UsageEnvironment *env, CVI_RTSP_CONFIG *config, UserAuthenticationDatabase *authDatabase) {
    ...
    return new CVI_RTSP(*env, ourSocket, port, timeout, *config, authDatabase);
}

CVI_RTSP::CVI_RTSP(UsageEnvironment& env, int ourSocket, Port ourPort, unsigned timeout, const CVI_RTSP_CONFIG &config, UserAuthenticationDatabase *authDatabase): RTSPServer(env, ourSocket, ourPort, authDatabase, timeout)
{
    onConnect = NULL;
    onDisconnect = NULL;
    _argConn = NULL;
