...
int deinitVideo(void) {
    ...
    ret = app_ipcam_Venc_Stop(APP_VENC_ALL);
    if (ret != CVI_SUCCESS) {
        printf("[video] app_ipcam_Venc_Stop failed: %#x\n", ret);
        if (first_error == CVI_SUCCESS) {
            first_error = ret;
        }
    }

    ret = app_ipcam_Vpss_DeInit();
    if (ret != CVI_SUCCESS) {
        printf("[video] app_ipcam_Vpss_DeInit failed: %#x\n", ret);
        if (first_error == CVI_SUCCESS) {
            first_error = ret;
        }
    }

    ret = app_ipcam_Vi_DeInit();
    if (ret != CVI_SUCCESS) {
        printf("[video] app_ipcam_Vi_DeInit failed: %#x\n", ret);
        if (first_error == CVI_SUCCESS) {
            first_error = ret;
        }
    }

    ret = app_ipcam_Sys_DeInit();
    if (ret != CVI_SUCCESS) {
        printf("[video] app_ipcam_Sys_DeInit failed: %#x\n", ret);
        if (first_error == CVI_SUCCESS) {
            first_error = ret;
        }
    }
